...
for(i=1; i<=driftnumber; i++){
  ...
  Message=Message+"Stack ends at frame"+(driftstart[driftnumber]-1)+"; Will rotate "+rotation+" to correct direction";
showMessage(Message);
...
refavrstart=driftstart[i-1]-48;
refavrstop=driftstart[i-1]-1;
driftavrstart=driftstart[i-1];
driftavrstop=driftstart[i-1]+47;

//Make a pair of 48-frame average images for feature extraction and alignment
